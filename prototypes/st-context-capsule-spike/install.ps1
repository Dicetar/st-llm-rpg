...
$target = Join-Path $stRoot 'public\scripts\extensions\third-party\st-rpg-context-capsule-spike'
...
if (Test-Path -LiteralPath $target) {
    ...
    if ($manifest.display_name -ne 'RPG Context Capsule Model Lab') {
...
Copy-Item -LiteralPath (Join-Path $source 'variants.js') -Destination $target
...
Write-Output "Installed RPG Context Capsule Model Lab to $target"
Write-Output 'Refresh SillyTavern and use the floating C button.'
