...
Issue #39 is active. Verified backup/restore shipped in preview.6; reviewable manifest-bound addon reconciliation shipped in preview.7; the identity-safe visible supervisor and CLI backup/restore shipped in preview.8; lock-driven staged pinned-SillyTavern verification shipped in preview.9 with sanitized live evidence in `docs/evidence/production-compatibility-stage-desktop-2026-08-10.json`; explicit backup/export/divergence-safe fallback and verified-Binding companion modes ship in preview.10 with a live desktop round-trip in `docs/evidence/production-mode-roundtrip-desktop-2026-08-10.json`. Complete the #39 acceptance audit before moving the frontier to #40. Changed-pin state transfer, injected post-switch rollback, and the physical-phone cutover remain #40 evidence rather than claimed preview.10 acceptance.
