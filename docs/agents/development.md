...
Issue #38 is active. Durable Story Sync jobs and editable source-linked Proposals live in SQLite, use a separate configurable Campaign Worker on the shared narrator-priority inference lane, and appear in Campaign Book's Review Inbox. The worker has read-only Campaign authority and cannot mutate Campaign truth. Explicit human finalization applies accepted Operations as one Campaign Event and advances the originating Binding's Sync Boundary as one Binding Event in the same SQLite transaction; completed source prose is pruned. Stop, evidence-checked resume/retry, and discard are implemented; discard prunes unresolved source/proposals without changing Campaign or Binding authority. Run a representative live-model Campaign trace before closing #38.
