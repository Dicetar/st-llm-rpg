...
Issues #32 through #39 are complete. Issue #40 is active. Preview.10 shipped explicit backup/export/divergence-safe fallback and verified-Binding companion modes with a live desktop round-trip in `docs/evidence/production-mode-roundtrip-desktop-2026-08-10.json`. Preview.11 adds explicit fresh saved-chat linking inside the selected Campaign; linking is never automatic and fallback-bearing chats remain in the reviewed import path. Changed-pin state transfer, injected post-switch rollback, full real-Campaign cutover trace, performance/resource evidence, and physical-phone cutover remain #40 gates.
