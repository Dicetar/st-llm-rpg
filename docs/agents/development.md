...
Issues #32 through #39 are complete. Issue #40 remains the owner-deferred physical-phone cutover gate. Preview.10 shipped explicit backup/export/divergence-safe fallback and verified-Binding companion modes with a live desktop round-trip in `docs/evidence/production-mode-roundtrip-desktop-2026-08-10.json`. Preview.11 adds explicit fresh saved-chat linking inside the selected Campaign; linking is never automatic and fallback-bearing chats remain in the reviewed import path. Preview.14 adds actual-host authority/resource measurements and an isolated real changed-revision switch with injected post-switch rollback. Preview.15 adds explicit per-chat **Follow current Campaign** reconciliation and passes the representative saved-chat desktop continued-play trace in `docs/evidence/production-real-campaign-cutover-desktop-2026-08-10.json`. Preview.16 adds Facts and World Objects across SQLite authority, inline Workspace editing, Scene attachment, deterministic retrieval, Story Sync proposals, and reviewed JSON addons. Preview.17 adds guarded **Advance Scene** as one accepted mutation that closes the current Scene into an immutable archive and opens an editable next Scene with explicit outcomes, unresolved threads, and carry-forward attachments. Preview.18 begins the Workspace decomposition and adds Campaign-wide search, Quick Capture, draft guards, and bounded lists. Preview.19 completes the first behavior-preserving Workspace split into focused modules, makes deterministic Session Home the Campaign entry point, keeps quick and detailed creation together, adds the illustrated Player Guide, and replaces leaked engineering vocabulary and raw primary errors with player-facing copy and recovery. Preview.20 ships issue #42 Actor Live Trackers and the Relationship Map: stable-ID Tracker operations and SQLite migration, legacy-meter conversion, bounded selected-Actor context, same-Actor CRUD and quick adjustments, current-Scene recap state, deterministic diagram, and keyboard/360px route list. Preview.21 ships issue #43's player-facing handbook: a task-led first-session checklist, normal play loop, direct shortcuts, context/privacy/saving explanations, recovery guidance, a standalone illustrated guide, and production-build desktop/360px browser evidence. Preview.22 ships issue #44's Campaign lifecycle and cutover-candidate browser journey: active/archive/restore authority, exact-revision branching with lineage, content-safe current JSON export, grouped Play/Library/Automation/Campaign navigation, v12-to-v13 backup/migration coverage, and repeatable desktop/360px journey evidence. Full portable JSON re-import and the physical-phone cutover remain open; keep fallback installed.
