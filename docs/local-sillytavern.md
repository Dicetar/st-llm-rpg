...
`npm run verify:compatibility-rollback` is a non-production drill. It clones the live pinned runtime below `.runtime/verification/`, switches that isolated copy to a different locally available revision, verifies it on port `18001`, injects a failure after the switch, and requires exact revision plus persistent-state-sentinel recovery. It does not stop or modify live SillyTavern and does not touch Campaign data.
