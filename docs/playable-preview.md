...
2. Start in the **Play** group at **Session Home**. It deterministically composes the present Scene, last closed Scene, active Quests, outcomes, and unresolved threads from the loaded SQLite revision; it makes no model call and changes nothing. **Library** contains durable records, **Automation** contains Narrator Context and Story Updates, and **Campaign** contains history and the Player Handbook. The same player-facing tour is available outside the app in [`docs/player-guide.md`](player-guide.md).
3. Use **Find anything** to search names, aliases, descriptions, Relationships, and Scene notes across the selected Campaign. Search works against the already-loaded authoritative revision, includes private player data without creating another persistent index, and can optionally include archived Records. Open any ordinary collection and use **Quick add** to capture a named stub immediately; expand **Add with details** in the same block when you need the full form. Large collections render in bounded pages while search still covers every Record.
4. Edit Actors, Items, Abilities, directed Relationships, Quests, Facts, Places, World Objects, and the Current Scene. On an Actor, add **Live trackers** for health, currency, charges, suspicion, or any other changing whole number. `−` and `+` save immediately; **Edit tracker** uses guarded Save/Cancel and keeps removal in the same Actor block. Session Home shows trackers for Actors attached to the present Scene. Open **Relationships** for the current directed-link map and its keyboard/phone route list. An Ability page keeps its reusable definition and every Actor's learned/prepared/enabled/use state together. Facts can be created and edited beside the Record they describe; World Objects can be created and edited beside their Place, then attached structurally to the Current Scene. **Advance Scene** records the closing summary, outcomes, unresolved threads, and selected carry-forward attachments, then archives the old Scene and opens the next one as one accepted mutation. If an editor or Quick Capture contains unsaved input, route changes and browser exit warn before discarding it. Each accepted edit advances immutable Campaign history; stale tabs cannot overwrite a newer revision.
5. Open **Narrator Context**. If this chat is behind Campaign head after an edit, narration stays blocked until you explicitly choose **Follow current Campaign**; the choice advances only that chat's Campaign Anchor and creates a Binding Event. Confirm the exact narrator model profile, automatic budget, and any manual pins. Use **Build Context Plan** for an inspectable dry run with no model call.
6. In SillyTavern, select the same exact model ID as the saved narrator profile and chat normally. The bridge routes linked generations through one deterministic Context Plan and one LM Studio call. The complete answer is withheld until accepted, so Stop cannot leave partial companion text.
7. Open **Narration status** in Campaign Book when a reply fails. It shows active requests and only the latest outcome, elapsed time, a safe error message, and concrete recovery guidance. It never stores prompts, generated prose, or request history and resets when the Companion restarts.
8. Open **Story Updates** and save the separate Campaign Worker model ID. In a linked SillyTavern chat, choose **Sync Story** from the extensions menu. The Companion analyzes only the bounded unseen range and opens editable, evidence-linked suggestions. Accept or reject every suggestion, then choose **Apply reviewed updates**. Accepted changes become one Campaign revision and the same SQLite transaction advances only that chat's Sync Boundary.
9. Use **Stop analysis** when a worker must yield. Resume/retry rechecks the Campaign head, Binding, and Sync Boundary before using retained source. **Discard review** removes unresolved source/proposals and changes neither Campaign truth nor the Sync Boundary.
10. Open **Backups and Restore** in Campaign Book. Today’s verified daily backup appears automatically. Create labelled backups before risky edits; **Preview restore** verifies file identity, hash, and Campaign history before the destructive restore action becomes available. Restore creates another verified safety backup first.
11. Use **Lifecycle & portability** on Session Home to archive or restore the Campaign, create an independent branch from an accepted revision, or download current Campaign truth and its accepted history index as JSON. An archived Campaign remains readable and exportable but blocks edits, linked narration, and Story Updates. A branch keeps record IDs and source lineage but deliberately copies no Chat Bindings, pins, jobs, drafts, or diagnostics.
12. To bulk-author outside the browser, edit `campaign-content/*.json`, open **JSON addon inbox**, choose the target Campaign, and preview. Apply is enabled only for a blocker-free exact manifest diff; it creates a verified pre-import backup and one Campaign revision.
...
- Campaign archive/restore, exact-revision branching with explicit lineage, and current canonical JSON export;
- grouped Play, Library, Automation, and Campaign navigation with collection counts;
- Actor-owned Live trackers with stable IDs, optional maximums and notes, immediate `−`/`+` commits, guarded detailed edits, legacy-meter import, revision reconstruction, and restart-safe SQLite projections;
- deterministic Relationship Map with a desktop diagram, bounded visual Actor count, and an accessible full route list used for keyboard and narrow-phone navigation;
- tracker-aware Campaign search, present-Scene recap, and bounded narration context: tracker detail follows an already selected Actor and is never dumped globally;
- deterministic Session Home recap and illustrated in-app Player Handbook, with no extra model call or persistence path;
- CampaignWorkspace split into focused API, shell, collection-creation, record-editor, Actor, Scene, recap, guide, and error modules while the remaining controller owns loading and authoritative mutations;
- Campaign-wide human search across names, aliases, descriptions, Relationships, and Scene notes, with explicit archived inclusion and no second private-text persistence path;
- one-field Quick Capture and progressively disclosed detailed creation in the same block for every ordinary collection, guarded unsaved editor navigation, and bounded 40-Record list pages;
- player-facing visibility names, recovery-first failure messages with secondary technical details, and action-oriented empty states;
...
- durable bounded Story Sync jobs, a separate Campaign Worker profile, structured editable proposals for Actors, Items, Abilities, Relationships, Quests, Facts, Places, World Objects, and Current Scene, atomic human-only finalization, and explicit stop/resume/discard recovery;
...
- The permanent cutover-candidate journey passes through Campaign creation, Actor editing, Scene setup/advance, explicit Chat Binding, stale-write rejection, Context planning, reviewed Story Updates, export, branch lineage, archive/restore, and 360px no-overflow checks. Evidence is in `docs/evidence/workspace-cutover-candidate-browser-2026-08-13.json`. Its SillyTavern and worker boundaries are deterministic fixtures; it does not replace the real-model and pinned-ST traces listed below.
- Campaign JSON export is a portable, inspectable representation, but full Campaign JSON re-import is not implemented yet. Existing fallback import and reviewed additive JSON addons remain available; do not treat export as an identity-preserving restore mechanism.
- The Player Handbook's three direct routes, production-build desktop/no-overflow layout, 360px one-column setup, and 44px actions pass in `docs/evidence/workspace-player-handbook-browser-2026-08-13.json`. Live Tracker and Relationship Map evidence remains in `docs/evidence/workspace-live-session-state-browser-2026-08-13.json`; Session Home and guarded editor evidence remains in `docs/evidence/workspace-session-home-browser-2026-08-13.json`. This is desktop browser automation, not physical-phone acceptance.
...
- Rich fallback-only fields beyond the companion's Actors, Items, Abilities, Relationships, active/completed Quests, Facts, Places, World Objects, and simplified current Scene remain future work. Unsupported fields show explicit warnings.
