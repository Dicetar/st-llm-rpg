...
2. Start at **Session Home**. It deterministically composes the present Scene, last closed Scene, active Quests, outcomes, and unresolved threads from the loaded SQLite revision; it makes no model call and changes nothing. Open **Player Handbook** for the task-led first-session checklist, normal play loop, direct collection shortcuts, context/privacy/saving guidance, and recovery steps. The same player-facing tour is available outside the app in [`docs/player-guide.md`](player-guide.md).
...
11. To bulk-author outside the browser, edit `campaign-content/*.json`, open **JSON addon inbox**, choose the target Campaign, and preview. Apply is enabled only for a blocker-free exact manifest diff; it creates a verified pre-import backup and one Campaign revision.
