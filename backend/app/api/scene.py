...
def open_scene(payload: SceneOpenRequest, actor_id: str = Query(default="player"), save_id: str = Query(default="default")):
    scene_service = SceneService(create_repository(save_id=save_id))
    try:
...
def close_scene(payload: SceneCloseRequest, actor_id: str = Query(default="player"), save_id: str = Query(default="default")):
    scene_service = SceneService(create_repository(save_id=save_id))
    try:
...
def draft_scene_close_summary(payload: SceneDraftCloseSummaryRequest, actor_id: str = Query(default="player"), save_id: str = Query(default="default")):
    scene_service = SceneService(create_repository(save_id=save_id))
    try:
