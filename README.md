...
- **Playable preview:** `0.3.0-preview.18` starts pinned SillyTavern and the production companion through one `Wayfinder.cmd` supervisor. Blank or imported Campaigns can explicitly link saved SillyTavern chats in Campaign Book; linked narration, Campaign-wide human search, in-place Quick Capture, guarded unsaved drafts, bounded large lists, Campaign editing (including reusable Abilities, per-Actor learned state, directed Relationships, Facts, Place/Scene-attached World Objects, and guarded Scene advance with immutable archives), explicit per-chat **Follow current Campaign** reconciliation, Context Tray, content-free narration status, recoverable atomic Story Sync Review Inbox, reviewable JSON addon inbox, verified backup/restore catalog, immutable history, measured cutover budgets, changed-revision rollback verification, and explicit fallback/companion mode switch are available now.
